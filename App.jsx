...
function Dial({ size, durationSec, elapsedSec, draggable, accent, trackColor, onChange, onTapCenter, centerLabel, centerSub, hideNumbers, pulse, fresh }) {
  const maxValue = 3600;
  ...
  useEffect(() => { if (!dragging) setLiveSec(draggable ? durationSec : elapsedSec); }, [durationSec, elapsedSec, draggable, dragging]);
  ...
  const arcValue = dragging ? liveSec : draggable ? durationSec : elapsedSec;
  ...
  // a fast-sweeping "second hand" (one revolution per 60s) so movement stays visible even
  // when the minute-scale progress pie barely changes, and especially when numbers are hidden
  const secondHandDeg = (Math.floor(elapsedSec) % 60) / 60 * 360;
  const secondHandPoint = angleToPoint(secondHandDeg, r * 0.94);
  ...
  return (
    ...
    >
      ...
      {ticks}
      {!fresh && (
        <line x1={cx} y1={cy} x2={secondHandPoint.x} y2={secondHandPoint.y} stroke={hideNumbers ? accent : "#20242B"} strokeWidth={size * 0.012} strokeLinecap="round" opacity={0.55} />
      )}
      {draggable && <circle cx={handlePoint.x} cy={handlePoint.y} r={size * 0.045} fill="#fff" stroke={accent} strokeWidth={size * 0.012} />}
      {!hideNumbers && (
        <text x={cx} y={cy - size * 0.02} fontSize={size * 0.13} fontWeight="600" fill="#20242B" textAnchor="middle" dominantBaseline="middle" fontFamily="'Space Grotesk', inherit">
          {centerLabel}
        </text>
      )}
      {centerSub && (
...
export default function PomodoroApp() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      try {
        const payload = JSON.stringify({ settings, customDurations, accumulatedFocusMin, history, todos, reflections, cycleCount, visitDates });
  ...
  function handlePhaseComplete() {
    ...
    if (finishedPhase === "focus") {
      ...
      if (settings.reflectionEnabled) {
        setPendingReflection({ id: uid(), dateISO: todayISO(), time: new Date().toTimeString().slice(0, 5), text: "", photo: null });
  ...
  return (
    <div style={{ background: settings.bgColor, minHeight: "100vh", fontFamily: settings.fontFamily ? `'${settings.fontFamily}', Inter, sans-serif` : "Inter, sans-serif", color: "#20242B", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", maxWidth: 480, margin: "0 auto", width: "100%", paddingBottom: tab === "timer" ? 0 : 76 }}>
        {tab === "timer" && (
          <TimerScreen
            isLandscape={isLandscape} phase={phase} durationSec={durationSec} elapsedSec={elapsedSec}
            ...
            allowHideNumbers={settings.allowHideNumbers} allowHideText={settings.allowHideText}
          />
        )}
        {tab === "todos" && (
          <TodosScreen todos={todos} setTodos={setTodos} activeTodoId={activeTodoId} setActiveTodoId={setActiveTodoId} accent={accent} />
        )}
        {tab === "stats" && (
        ...
        {tab === "settings" && (
          <SettingsScreen
            ...
            onResetStats={resetStats} accent={accent}
      ...
      {showReflection && pendingReflection && (
        <ReflectionModal data={pendingReflection} setData={setPendingReflection} onSave={saveReflection} onSkip={skipReflection} accent={accent} />
      )}
    </div>
  );
}
...
function TimerScreen({ isLandscape, phase, durationSec, elapsedSec, fresh, running, timeLeft, accent, pulse, accumulatedFocusMin, bottomSentence, activeTodoTitle, todos, activeTodoId, setActiveTodoId, onDialChange, toggleRun, resetTimer, skipPhase, onSelectPhase, allowHideNumbers, allowHideText }) {
  const { w: vw, h: vh } = useViewportSize();
  // reserve space for nav bar (~68px) + surrounding chrome, then size the dial to whatever's left
  const navReserve = 68;
  const dialSize = isLandscape
    ? Math.max(120, Math.min(vh - navReserve - 96, vw * 0.32, 200))
    : Math.max(170, Math.min(vw * 0.6, vh - navReserve - 210, 250));
    ...
    <div style={{ display: "flex", gap: 6, justifyContent: "center" }}>
      {PHASE_ORDER.map((p) => (
        <button key={p} onClick={() => onSelectPhase(p)} disabled={running} style={{
          border: "none", borderRadius: 999, padding: "4px 10px", fontSize: 11, cursor: running ? "default" : "pointer",
  ...
  const dial = (
    <Dial size={dialSize} durationSec={durationSec} elapsedSec={elapsedSec} draggable={fresh} fresh={fresh} accent={accent}
      ...
      hideNumbers={hideNumbers} pulse={pulse} />
  );

  const hideToggles = (allowHideNumbers || allowHideText) && (
    <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
      {allowHideNumbers && (
        <button onClick={() => setHideNumbers((v) => !v)} style={{ ...iconBtnStyle(30), background: hideNumbers ? accent : "#fff" }}>
          <Hash size={13} color={hideNumbers ? "#fff" : "#9AA1AE"} />
        </button>
      )}
      {allowHideText && (
        <button onClick={() => setHideText((v) => !v)} style={{ ...iconBtnStyle(30), background: hideText ? accent : "#fff" }}>
          <Type size={13} color={hideText ? "#fff" : "#9AA1AE"} />
        </button>
      )}
    </div>
  );
  ...
  const controls = (
    <div style={{ display: "flex", flexDirection: "column", gap: 8, alignItems: "center" }}>
      <div style={{ display: "flex", gap: 12, alignItems: "center", justifyContent: "center" }}>
        <button onClick={resetTimer} style={iconBtnStyle(36)}><RotateCcw size={16} color="#6B7280" /></button>
        <button onClick={toggleRun} style={{ ...iconBtnStyle(48), background: accent, color: "#fff" }}>
          {running ? <Pause size={20} /> : <Play size={20} style={{ marginLeft: 2 }} />}
        </button>
        <button onClick={skipPhase} style={iconBtnStyle(36)}><SkipForward size={16} color="#6B7280" /></button>
      </div>
      {hideToggles}
    </div>
  );
  ...
  if (isLandscape) {
    return (
      <div style={{ display: "flex", flexDirection: "column", height: `calc(100dvh - ${navReserve}px)`, padding: "10px 18px 6px", overflow: "hidden" }}>
        ...
        <div style={{ flexShrink: 0, margin: "2px 0" }}>{phaseTabs}</div>
        <div style={{ flex: 1, minHeight: 0, display: "flex", alignItems: "center", justifyContent: "center", gap: 20 }}>
          {dial}
          {controls}
        </div>
        {!hideText && bottomSentence != null && (
          <div style={{ display: "flex", justifyContent: "space-between", flexShrink: 0, gap: 8 }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", height: `calc(100dvh - ${navReserve}px)`, padding: "10px 20px 8px", textAlign: "center", overflow: "hidden" }}>
      ...
      <div style={{ flexShrink: 0, margin: "6px 0" }}>{phaseTabs}</div>
      <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", gap: 14 }}>
        {dial}
        {controls}
      </div>
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(20,22,26,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 65, padding: 24 }}>
      <div style={{ background: "#fff", borderRadius: 16, padding: 22, width: "100%", maxWidth: 320 }}>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          ...
          <button onClick={() => onConfirm(Math.max(1, min * 60 + sec))} style={smallBtnStyle(true)}>확인</button>
        </div>
      </div>
    </div>
  );
...
function TaskPickerModal({ todos, activeTodoId, accent, onSelect, onClear, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(20,22,26,0.45)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 50 }} onClick={onClose}>
      <div style={{ background: "#fff", width: "100%", maxWidth: 480, borderRadius: "20px 20px 0 0", padding: 22, maxHeight: "70vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
...
function Bracketed({ children, small }) {
  return (
    <div style={{ fontSize: small ? 13 : 15, color: "#5B6270", fontWeight: 500, letterSpacing: 0.2 }}>
...
function TodosScreen({ todos, setTodos, activeTodoId, setActiveTodoId, accent }) {
  ...
  return (
    <div style={{ padding: "24px 20px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {todos.map((t) => (
          <div key={t.id} onClick={() => setActiveTodoId(t.id)} style={{
            ...
            <div style={{ fontSize: 12, color: "#9AA1AE", marginTop: 6, marginLeft: 30 }}>
              뽀모도로 {t.done} / {t.est}
              {activeTodoId === t.id && <span style={{ color: accent, marginLeft: 8, fontWeight: 600 }}>진행 중</span>}
            </div>
          </div>
        ))}
      </div>
...
function StatsScreen({ history, todos, reflections, accumulatedFocusMin, accent, onUpdateReflection, onDeleteReflection, onDeleteHistoryEntry }) {
  ...
  return (
    <div style={{ padding: "24px 20px" }}>
      <h2 style={headingStyle()}>통계</h2>

      <div style={{ display: "flex", gap: 10, marginBottom: 10 }}>
        <StatCard label="오늘 완료" value={`${todayCount}개`} accent={accent} />
        <StatCard label="이번 주 완료" value={`${weekCount}개`} accent={accent} />
        <StatCard label="누적 시간" value={formatMinutesLabel(accumulatedFocusMin)} accent={accent} />
      </div>
      ...
      {completedTodos.length === 0 ? (
        ...
        <ul style={{ margin: "0 0 22px", padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 6 }}>
          {completedTodos.map((t) => (
            <li key={t.id} style={{ fontSize: 13.5, color: "#5B6270", display: "flex", alignItems: "center", gap: 8 }}><Check size={13} color={accent} /> {t.title}</li>
          ))}
        </ul>
      )}

      <p style={subHeadingStyle()}>회고 기록</p>
...
function StatCard({ label, value, accent }) {
  return (
    <div style={{ flex: 1, border: "1px solid #EAEAE6", borderRadius: 12, padding: "12px 10px", textAlign: "center", background: "#fff" }}>
      <div style={{ fontSize: 17, fontWeight: 700, color: accent, fontFamily: "'Space Grotesk', inherit" }}>{value}</div>
...
function SettingsScreen({ settings, setSettings, customDurations, setCustomDurations, onResetStats, accent, storageStatus, onExport, onImport, importMessage, visitDates, onToggleVisit }) {
  ...
  return (
    <div style={{ padding: "24px 20px 40px" }}>
      <h2 style={headingStyle()}>설정</h2>

      <SettingSection title="시간">
        <MinSecRow label="집중" totalSec={customDurations.focus} onChangeMin={(v) => setDurationPart("focus", "min", v)} onChangeSec={(v) => setDurationPart("focus", "sec", v)} />
        <MinSecRow label="짧은 휴식" totalSec={customDurations.short} onChangeMin={(v) => setDurationPart("short", "min", v)} onChangeSec={(v) => setDurationPart("short", "sec", v)} />
        <MinSecRow label="긴 휴식" totalSec={customDurations.long} onChangeMin={(v) => setDurationPart("long", "min", v)} onChangeSec={(v) => setDurationPart("long", "sec", v)} />
        ...
        <p style={{ fontSize: 11.5, color: "#9AA1AE", margin: 0 }}>타이머 화면에서 집중/짧은 휴식/긴 휴식 탭을 눌러 각 시간을 미리 보고 다이얼로 직접 드래그 조절할 수 있어요.</p>
      </SettingSection>
          ...
          <>
            ...
            <RowShell label="미리 듣기">
              <button onClick={() => {
                ...
                a.play().catch(() => {});
                setTimeout(() => { try { a.pause(); } catch (e) {} }, (settings.customSoundDuration || 4) * 1000);
              }} style={smallBtnStyle()}>재생</button>
            </RowShell>
            <button onClick={() => update({ customSoundDataUrl: null, customSoundName: null })} style={{ background: "none", border: "none", color: "#B5533C", fontSize: 12, padding: 0, textAlign: "left", cursor: "pointer" }}>
      ...
      <SettingSection title="화면">
        ...
        <ToggleRow label="타이머 실행 중 숫자 숨기기 버튼 사용" checked={settings.allowHideNumbers} onChange={(v) => update({ allowHideNumbers: v })} accent={accent} />
        <ToggleRow label="타이머 실행 중 텍스트 숨기기 버튼 사용" checked={settings.allowHideText} onChange={(v) => update({ allowHideText: v })} accent={accent} />
      </SettingSection>
...
function ColorPickerModal({ title, value, defaultValue, onConfirm, onCancel }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(20,22,26,0.45)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 70 }}>
      <div style={{ background: "#fff", width: "100%", maxWidth: 480, borderRadius: "20px 20px 0 0", padding: 22, maxHeight: "85vh", overflowY: "auto" }}>
...
function MinSecRow({ label, totalSec, onChangeMin, onChangeSec }) {
...
function ReflectionModal({ data, setData, onSave, onSkip, accent }) {
  const onPickPhoto = (e) => {
    const f = e.target.files?.[0]; if (!f) return;
    const reader = new FileReader();
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(20,22,26,0.45)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 50 }}>
      <div style={{ background: "#fff", width: "100%", maxWidth: 480, borderRadius: "20px 20px 0 0", padding: 22 }}>
...
function headingStyle() { return { fontSize: 20, fontWeight: 700, margin: "0 0 20px", fontFamily: "'Space Grotesk', inherit" }; }
