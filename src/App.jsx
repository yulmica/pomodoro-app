...
export default function PomodoroApp() {
  ...
  // 리셋 버튼: 기존에 설정해둔 시간으로 되돌리는 대신, 무조건 00:00으로 초기화해서
  // 다이얼에서 바로 새 시간을 다시 설정할 수 있게 한다.
  const resetTimer = () => {
    commitPartialFocusIfEligible();
    setRunning(false);
    setCustomDurations((d) => ({ ...d, [phase]: 0 }));
    setTimeLeft(0);
  };
  const skipPhase = () => {
    commitPartialFocusIfEligible();
    ...
    <div style={{ background: settings.bgColor, minHeight: "100vh", fontFamily: settings.fontFamily ? `'${settings.fontFamily}', Inter, sans-serif` : "Inter, sans-serif", color: "#20242B", display: "flex", flexDirection: "column" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        /* 브라우저 기본값은 button/input/textarea/select에 앱 폰트를 자동으로 물려주지 않는다.
           이 규칙이 없으면 제목이나 숫자는 커스텀 폰트로 바뀌어도 버튼 텍스트(탭, 설정 버튼 등)는
           계속 시스템 기본 폰트로 남아 "폰트가 안 바뀐다"는 문제가 생긴다. */
  ...
  return (
    <div style={{ padding: "24px 20px 40px" }}>
      ...
      <SettingSection title="시간">
        ...
        <NumberRow label="긴 휴식 전 집중 횟수" value={settings.cyclesBeforeLong} onChange={(v) => update({ cyclesBeforeLong: v })} min={1} />
      ...
      <SettingSection title="알림">
        ...
        {settings.customSoundDataUrl && (
          <>
            <NumberRow label="재생 길이 (초)" value={settings.customSoundDuration} onChange={(v) => update({ customSoundDuration: v })} min={1} max={30} />
...
/* NumberRow: 로컬 텍스트 상태로 편집하다가, 입력을 끝냈을 때(blur/Enter)만 min/max로
   보정한다. 그래야 기존 값(예: 4)을 지우고 0부터 새 숫자를 다시 입력하는 도중에
   값이 강제로 1로 튕기는 문제 없이 자유롭게 타이핑할 수 있다. */
function NumberRow({ label, value, onChange, min = 0, max }) {
