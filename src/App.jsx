...
function Dial({ size, durationSec, elapsedSec, draggable, accent, trackColor, onChange, centerLabel, centerSub, hideNumbers, pulse }) {
  const maxValue = 3600;
...
export default function PomodoroApp() {
  ...
  const isLandscape = settings.forceLandscape || isLandscapeViewport;
  ...
  useEffect(() => {
    ...
    link.href = `https://fonts.googleapis.com/css2?family=${encodeURIComponent(settings.fontFamily.trim())}:wght@400;500;600;700&display=swap`;
  }, [settings.fontFamily]);
...
function TimerScreen({ isLandscape, phase, durationSec, elapsedSec, fresh, running, timeLeft, accent, pulse, accumulatedFocusMin, bottomSentence, activeTodoTitle, todos, activeTodoId, setActiveTodoId, onDialChange, toggleRun, resetTimer, skipPhase, onSelectPhase, allowHideNumbers, allowHideText }) {
  const dialSize = isLandscape ? 220 : 260;
  ...
  const taskLabel = (
    <button onClick={() => setShowPicker(true)} style={{ background: "none", border: "none", padding: 0, cursor: "pointer" }}>
      <Bracketed small={isLandscape}>{hideText ? "•••" : (activeTodoTitle || "할 일을 선택하세요")}</Bracketed>
    </button>
  );
    ...
    <div style={{ display: "flex", gap: 6, justifyContent: "center", marginBottom: 4 }}>
      {PHASE_ORDER.map((p) => (
        <button key={p} onClick={() => onSelectPhase(p)} disabled={running} style={{
          border: "none", borderRadius: 999, padding: "5px 11px", fontSize: 11.5, cursor: running ? "default" : "pointer",
  ...
  const dial = (
    <Dial size={dialSize} durationSec={durationSec} elapsedSec={elapsedSec} draggable={fresh} accent={accent}
      trackColor="#EDEEEA" onChange={onDialChange} centerLabel={clock} centerSub={hideText ? null : PHASES[phase].label}
  ...
  const hideToggles = (allowHideNumbers || allowHideText) && (
    <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
      {allowHideNumbers && (
        <button onClick={() => setHideNumbers((v) => !v)} style={{ ...iconBtnStyle(32), background: hideNumbers ? accent : "#fff" }}>
          <Hash size={14} color={hideNumbers ? "#fff" : "#9AA1AE"} />
        </button>
      )}
      {allowHideText && (
        <button onClick={() => setHideText((v) => !v)} style={{ ...iconBtnStyle(32), background: hideText ? accent : "#fff" }}>
          <Type size={14} color={hideText ? "#fff" : "#9AA1AE"} />
        </button>
      )}
    </div>
  );
  ...
  const controls = (
    <div style={{ display: "flex", flexDirection: "column", gap: 10, alignItems: "center" }}>
      <div style={{ display: "flex", gap: 14, alignItems: "center", justifyContent: "center" }}>
        <button onClick={resetTimer} style={iconBtnStyle()}><RotateCcw size={18} color="#6B7280" /></button>
        <button onClick={toggleRun} style={{ ...iconBtnStyle(52), background: accent, color: "#fff" }}>
          {running ? <Pause size={22} /> : <Play size={22} style={{ marginLeft: 2 }} />}
        </button>
        <button onClick={skipPhase} style={iconBtnStyle()}><SkipForward size={18} color="#6B7280" /></button>
      </div>
      {hideToggles}
    </div>
  );
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", flex: 1, padding: "20px 22px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <Bracketed small>Pomo</Bracketed>
          {taskLabel}
        </div>
        {phaseTabs}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1, padding: "28px 24px 12px", textAlign: "center" }}>
      {taskLabel}
      {phaseTabs}
    ...
    <div style={{ padding: "24px 20px 40px" }}>
      ...
      <SettingSection title="알림">
        ...
        <RowShell label="알림음 파일">
          <FileLabel accept="audio/*" onFile={(e) => { const f = e.target.files?.[0]; if (f) update({ customSoundName: f.name }); }}>
      ...
      <SettingSection title="화면">
        ...
        <RowShell label="앱 폰트 (Google Fonts 이름)" full>
          <input value={settings.fontFamily} onChange={(e) => update({ fontFamily: e.target.value })} placeholder="예: Nanum Gothic, Gowun Dodum" style={inputStyle()} />
        </RowShell>
        ...
        <ToggleRow label="가로모드 강제 사용" checked={settings.forceLandscape} onChange={(v) => update({ forceLandscape: v })} accent={accent} />
